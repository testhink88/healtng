...
const ClinicInventoryManagement = () => {
  ...
  const categories = useMemo(
    () => [...new Set(inventoryItems.map((item) => item?.categoria).filter(Boolean))],
    [inventoryItems]
  );
  const locations = useMemo(
    () => [...new Set(inventoryItems.map((item) => item?.ubicacion).filter(Boolean))],
    [inventoryItems]
  );
  ...
  const handleExportCSV = () => {
    const csvContent =
      ...
      "Código,Nombre,Categoría,Stock Actual,Stock Mínimo,Ubicación,Proveedor,Costo,Vencimiento,Lote\n" +
      filteredItems
        ?.map(
          (item) =>
  ...
  // ✅ (3) Resumen ejecutivo
  const inventoryExecutive = useMemo(() => {
    const now = new Date();
    const in60 = new Date();
    in60.setDate(in60.getDate() + 60);

    const totalValue = inventoryItems.reduce((acc, it) => {
      const cost = safeNum(it?.costo_unitario, 0);
      const qty = safeNum(it?.stock_actual, 0);
      return acc + cost * qty;
    }, 0);

    const expRiskValue = inventoryItems.reduce((acc, it) => {
      if (!it?.vencimiento) return acc;
      const exp = new Date(it.vencimiento);
      if (Number.isNaN(exp.getTime())) return acc;

      // riesgo: vence dentro de 60 días (y aún no vencido)
      if (exp >= now && exp <= in60) {
        const cost = safeNum(it?.costo_unitario, 0);
        const qty = safeNum(it?.stock_actual, 0);
        return acc + cost * qty;
    ...
    const lowStockItems = inventoryItems.filter((it) => isLowStock(it));

    // Impacto demo: si el item trae proceduresImpact/procedures_impact úsalo; si no, heurística por mínimo
    const criticalProceduresImpact = lowStockItems.reduce((acc, it) => {
      const explicit = safeNum(it?.proceduresImpact ?? it?.procedures_impact, NaN);
      if (Number.isFinite(explicit)) return acc + explicit;

      const min = safeNum(it?.stock_mínimo, 1);
      // heurística: mínimo stock ~ procedimientos comprometidos
      return acc + Math.max(1, Math.round(min));
    }, 0);

    return {
      totalValue,
      expRiskValue,
      criticalProceduresImpact,
      ...
      >
          ...
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div
              className={`h-2 rounded-full transition-all ${
                isLowStock(item)
                  ? "bg-red-500"
                  : item?.stock_actual <= item?.stock_mínimo * 1.5
                  ? "bg-yellow-500"
                  : "bg-green-500"
              }`}
              style={{
                width: `${Math.min((safeNum(item?.stock_actual, 0) / safeNum(item?.stock_máximo, 1)) * 100, 100)}%`,
              }}
            />
          </div>
        </div>
        ...
        <div className="text-xs text-gray-500 space-y-1">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium">${safeNum(item?.costo_unitario, 0)}</span>
          </div>
        ...
        <div className="p-6">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Button variant="outline" onClick={() => navigate("/marketplace/b2b")} className="flex items-center space-x-2">
                <Icon name="Store" size={16} />
                <span>Surtir desde B2B</span>
              </Button>

              <Button onClick={() => navigate("/clinic/inventory/new")} className="flex items-center space-x-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <p className="text-xs text-gray-400 uppercase tracking-wide">Valor total inventario</p>
              <p className="text-2xl font-bold text-gray-900">
                ${inventoryExecutive.totalValue.toFixed(2)}
              </p>
              <p className="text-xs text-gray-500 mt-1">Estimado en base a costo unitario × stock</p>
            </div>

            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <p className="text-xs text-gray-400 uppercase tracking-wide">Riesgo por vencimiento (60 días)</p>
              <p className="text-2xl font-bold text-gray-900">
                ${inventoryExecutive.expRiskValue.toFixed(2)}
              </p>
              <p className="text-xs text-gray-500 mt-1">Productos por vencer (no vencidos)</p>
            </div>

            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <p className="text-xs text-gray-400 uppercase tracking-wide">Stock crítico impacta</p>
              <p className="text-2xl font-bold text-gray-900">
                {inventoryExecutive.criticalProceduresImpact} procedimientos
              </p>
              <p className="text-xs text-gray-500 mt-1">
                {inventoryExecutive.lowStockCount} insumos en estado crítico
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pt-4 border-t border-gray-100">
              <div className="flex items-center space-x-4 mb-4 sm:mb-0">
                <span className="text-sm text-gray-500">{filteredItems?.length} productos encontrados</span>
                {selectedItems?.length > 0 && <span className="text-sm font-medium text-primary">{selectedItems?.length} seleccionados</span>}
              </div>
                ...
                <div className="bg-white border border-gray-200 rounded-t-lg p-4">
                  <label className="flex items-center space-x-3">
                    <input
                      ...
                      className="rounded border-gray-300 text-primary focus:ring-primary"
                    />
                    <span className="text-sm font-medium text-gray-700">Seleccionar todos en esta página</span>
                  </label>
                </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between bg-white border border-gray-200 rounded-lg p-4">
              <div className="text-sm text-gray-500">
                ...
                {Math.min(currentPage * itemsPerPage, filteredItems?.length)} de {filteredItems?.length} productos
              </div>
