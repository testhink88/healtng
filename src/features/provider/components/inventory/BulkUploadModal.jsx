...
const BulkUploadModal = ({ isOpen, onClose, onUploadComplete }) => {
  ...
  const handleFileUpload = (file) => {
    ...
    reader.onload = (e) => {
      ...
      const idxUnit = find("unit", "unidad");
    ...
    <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-lg shadow-lg max-w-2xl w-full mx-4">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h2 className="text-lg font-semibold">Importar catálogo desde CSV</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          <p className="text-sm text-gray-500 mb-4">
            Sube tu archivo de productos para agregarlos al inventario. Lo ideal es que incluya
            al menos <strong>nombre</strong>, <strong>SKU</strong>, <strong>stock</strong> y{" "}
            <strong>precio</strong>. Otros campos como categoría, MOQ o precio mayorista
            se mapearán automáticamente si existen.
          </p>

          <BulkUploadSection onFileUpload={handleFileUpload} />

          <p className="text-xs text-gray-400 mt-4">
            Tip: si ya usas otra plataforma, exporta tu catálogo en CSV y súbelo aquí. Healtng se
            encargará de normalizar las columnas más comunes.
          </p>
        </div>
